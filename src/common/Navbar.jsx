...
const Navbar = ({ isFullscreenNav, setIsFullscreenNav }) => {
  const [stroke, setStroke] = useState("#ffffff")
  const hoverDiv = useRef(null)
  const svgRef = useRef(null)

  return (
    <div className="flex flex-row justify-between z-3 fixed top-0 w-full">
      ...
      {!isFullscreenNav && <div onMouseEnter={() => {
        ...
        className="w-[16vw] h-12 bg-black relative hover:cursor-pointer">
        <div ref={hoverDiv} className="bg-[#D3FD50] h-0 w-full transition-all"></div>
        <div className=""> </div>
        <div onClick={() => {
          setStroke("#ffffff")
          setIsFullscreenNav(true)
        }} className="w-full h-full flex justify-end pr-4 pt-2 text-2xl absolute top-0">
